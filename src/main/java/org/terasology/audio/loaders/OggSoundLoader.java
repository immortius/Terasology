 ...
 * @author Immortius
 */
public class OggSoundLoader implements AssetLoader<SoundData> {

    @Override
    public SoundData load(AssetUri uri, InputStream stream, List<URL> urls) throws IOException {
        try (OggReader reader = new OggReader(stream)) {
            ...
            return new SoundData(data, reader.getChannels(), reader.getRate(), 16);
        } catch (IOException e) {
