...
public class PlayerFactory {
    ...
    public EntityRef newInstance(Vector3f spawnPosition) {
        ...
        AABBCollisionComponent collision = player.addComponent(new AABBCollisionComponent(new Vector3f(.3f, 0.8f, .3f)));

        CharacterMovementComponent movementComp = new CharacterMovementComponent();
        movementComp.groundFriction = 16f;
        movementComp.maxGroundSpeed = 3f;
        movementComp.distanceBetweenFootsteps = 1.5f;
        player.addComponent(movementComp);

        CharacterSoundComponent sounds = new CharacterSoundComponent();
        sounds.footstepSounds.addAll(Arrays.asList(AudioManager.sounds("FootGrass1", "FootGrass2", "FootGrass3", "FootGrass4", "FootGrass5")));
        player.addComponent(sounds);
